"""Analyze distribution of market event-types by volume with hierarchical groupings."""
...
from src.analysis.kalshi.util.categories import EVENT_CATEGORIES_COLORS, get_group, get_hierarchy
from src.common.analysis import Analysis, AnalysisOutput
from src.common.interfaces.chart import ChartConfig, ChartType, UnitType


class MarketEventCategoriesAnalysis(Analysis):
    """Analyze market/event category distribution by volume on Kalshi."""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            description="Market event-category distribution by notional volume",
        )
    ...
    def run(self) -> AnalysisOutput:
        ...
        # Get event categories with market count
        df_raw = con.execute(
            f"""
            SELECT
    ...
    def _create_figure(self, df_raw: pd.DataFrame, df_grouped: pd.DataFrame) -> plt.Figure:
        ...
        for _, row in df_treemap.iterrows():
            base = EVENT_CATEGORIES_COLORS.get(row["group"], "#888888")
        ...
        legend_elements = [
            Patch(facecolor=EVENT_CATEGORIES_COLORS[g], label=g) for g in df_grouped["group"].tolist() if g in EVENT_CATEGORIES_COLORS
        ]
        ax.legend(
