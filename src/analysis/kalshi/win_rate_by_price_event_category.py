...
class WinRateByPriceEventCategoryAnalysis(Analysis):
    ...
    def __init__(
        ...
        super().__init__(
            name="win_rate_by_price_event_category",
            description="Win rate vs price market calibration analysis(by event category)",
        )
    ...
    def _create_chart(self, df: pd.DataFrame) -> ChartConfig:
        ...
        return ChartConfig(
            ...
            title=f"Actual Win Rate vs Contract Price For {self.name.split('_')[-1].replace('_', ' ').title()}",
