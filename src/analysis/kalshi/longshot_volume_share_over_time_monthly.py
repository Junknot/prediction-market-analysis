...
class LongshotVolumeShareOverTimeMonthlyAnalysis(Analysis):
    """Analyze longshot volume share over time(monthly) on Kalshi."""
    ...
    def __init__(
        ...
        super().__init__(
            name="longshot_volume_share_over_time_monthly",
            description="Taker volume share in longshot contracts (1-20c) by month",
        )
    ...
    def run(self) -> AnalysisOutput:
        ...
        df = con.execute(
            ...
            SELECT
                ...
                COUNT(*) AS n_trades
            FROM bucketed
            GROUP BY month, price_bucket, is_longshot
            ORDER BY month, price_bucket
            """
        ).df()

        # Convert month to pandas datetime
        df["month"] = pd.to_datetime(df["month"])

        # Compute total volume per month
        monthly_totals = df.groupby("month")["volume_usd"].sum().reset_index()
    ...
    def _create_figure(self, longshot_df: pd.DataFrame) -> plt.Figure:
        ...
        months = longshot_df["month"].values
        x = np.arange(len(months))
        month_labels = [f"{pd.Timestamp(m).year}-{pd.Timestamp(m).month:02d}" for m in months]
    ...
    def _create_chart(
        ...
        pivot_df = df.pivot_table(index="month", columns="price_bucket", values="volume_share", aggfunc="sum").fillna(
            0
        )
        ...
        # Filter out months with < $1M volume
        valid_months = set(pd.to_datetime(monthly_totals[monthly_totals["total_volume"] >= 1e6]["month"]))
        pivot_filtered = pivot_df[pivot_df.index.isin(valid_months)]
        ...
        chart_data = [
            {
                "month": f"{pd.Timestamp(m).year}-{pd.Timestamp(m).month:02d}",
                **{
                    bucket_key_map[bucket]: round(pivot_filtered.loc[m, bucket], 2)
                    if bucket in pivot_filtered.columns
                    else 0
                    for bucket in bucket_order
                },
            }
            for m in pivot_filtered.index
        ]
