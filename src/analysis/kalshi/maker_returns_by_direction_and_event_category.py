...
class MakerReturnsByDirectionEventCategoryAnalysis(Analysis):
    """Analyze maker returns by position direction (YES vs NO) and event category."""
    ...
    def __init__(
        ...
        super().__init__(
            name="maker_returns_by_direction_event_category",
            description="Maker excess returns by position direction (YES vs NO) and event category",
        )
    ...
    def run(self) -> AnalysisOutput:
        ...
        date_range_qry_str, self.start_date, self.end_date = self._get_date_range_query_string()

        self.name = f"maker_returns_by_direction_event_category_{category.replace(' ', '_').lower()}"
    ...
    def _create_figure(self, df: pd.DataFrame) -> plt.Figure:
        ...
        if self.start_date and self.end_date:
            ax.set_title(
                f"Maker Excess Returns by Position Direction\nEvent Category: {category_label}, {self.start_date.strftime('%Y-%m-%d')} to {self.end_date.strftime('%Y-%m-%d')}"
            )
        elif self.start_date:
            ax.set_title(
                f"Maker Excess Returns by Position Direction\nEvent Category: {category_label}, from {self.start_date.strftime('%Y-%m-%d')}"
            )
        elif self.end_date:
            ax.set_title(
                f"Maker Excess Returns by Position Direction\nEvent Category: {category_label}, up to {self.end_date.strftime('%Y-%m-%d')}"
            )
        else:
